...
# GitHub Releases API로 버전 체크 (비공개 레포여도 Releases는 공개 가능)
...
def check_update():
    try:
        ...
        notes = data.get("body", "")
        # 다운로드 URL 찾기 (가계부.exe 또는 installer)
        download_url = ""
        installer_url = ""
...
def download_and_update(window, latest_version, download_url):
    try:
        ...
        ext = ".exe"
        tmp_exe = os.path.join(tmp_dir, "update" + ext)
...
def on_loaded(window, api):
    ...
    def bg_check():
        ...
        if latest:
            ...
            window.evaluate_js(
                "showUpdateNotify('" + latest + "', '" + safe_notes + "')"
            )
